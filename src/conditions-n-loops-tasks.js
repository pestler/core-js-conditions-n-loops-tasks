...
function getBalanceIndex(arr) {
  ...
  for (let y = 0; y < arr.length; y += 1) {
    if (leftSumma === rightSumma) return y;
    leftSumma += arr[y];
    rightSumma -= arr[y + 1];
  }
  return -1;
...
function getSpiralMatrix(size) {
  ...
  for (let i = 0; i <= size * size - 1; i += 1) {
    matrix[a][b] = i + 1;
    if (a === min && b < max) b += 1;
    else if (a < max && b === max) a += 1;
    else if (a > min && b === min - 1) a -= 1;
    else if (a === max && b > min) b -= 1;
    else if (a === max && b === min) {
      min += 1;
      max -= 1;
      a -= 1;
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < lengthMatrix; i += 1) {
    result[i] = [];
    for (let j = 0; j < lengthMatrix; j += 1) {
      result[i][j] = matrix[i][j];
    }
  }

  for (let k = 0; k < lengthMatrix; k += 1) {
    for (let l = lengthMatrix - 1; l >= 0; l -= 1) {
      copyMatrix[l][k] = result[lengthMatrix - k - 1][l];
    }
  }
